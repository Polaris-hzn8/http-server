...
int initListenFd(unsigned short port) {
	...
	int reuse = 1; setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, (char *)&reuse, sizeof(int));//设置端口复用
	struct sockaddr_in addr;
	addr.sin_family = AF_INET;//ipv4
	addr.sin_port = htons(port);//port 本地字节序转为网络字节序（大端）
	addr.sin_addr.s_addr = INADDR_ANY;//ip地址（大端）INADDR_ANY监听任何一个地址 0.0.0.0
	if (bind(sockfd, (struct sockarr*)&addr, sizeof(addr)) < 0) return -1;

	//3.设置监听套接字 将主动套接字转为被动套接字
	if (listen(sockfd, 128) < 0) return -1;// 128: 正在连接的连接序列有多少个 一次性能够最多接受多少个连接
	return sockfd;
}
